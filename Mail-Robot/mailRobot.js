...
const compareRobots = () => {
	let theRobot = [0, 0, 0, 0];
	let state = VillageState.random();
	for (let i = 0; i < 100; i++) {
		theRobot[0] += countTurns(state, randomRobot, []);
		theRobot[1] += countTurns(state, routeRobot, []);
		theRobot[2] += countTurns(state, goalOrientedRobot, []);
		theRobot[3] += countTurns(state, smarterRobot, []);
	}
	console.log(
