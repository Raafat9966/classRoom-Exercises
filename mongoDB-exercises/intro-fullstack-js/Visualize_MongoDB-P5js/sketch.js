...
function setup() {
	...
	// - The API key
	url =
		"https://webhooks.mongodb-realm.com/api/client/v2.0/app/visualizingdb-mtilm/service/realm-exercise/incoming_webhook/webhook0";
	...
	for (var i = 0; i < data.length; i++) {
		...
		if (i > 0) {
			line(
				data[i].x.$numberInt,
				data[i].y.$numberInt,
				data[i - 1].x.$numberInt,
				data[i - 1].y.$numberInt
			);
		}
	}
}
